...
def main() -> int:
    ...
    t0 = time.time()
    print(f"[run_pipeline] run_id={run_id}")
    external_data = False
    if args.data_dir:
        src_data = os.path.abspath(args.data_dir)
        external_data = True
        print(f"[run_pipeline] external mapped dataset: {src_data}")
        ...
        gen.write(data_dir)
        print(f"[run_pipeline] data prepared at {data_dir}")
    ...
    for table, fn in (
        ...
        with open(path, encoding="utf-8") as f:
            if external_data:
                ...
                res = fn(f.read())
        print(f"[run_pipeline] {res.report()}")
    ...
    print(f"[run_pipeline] expected dirty cases intercepted: {len(expected_dirty)}")
    receipt = {
        "run_id": run_id,
        "mode": "STUDY",
    ...
    print(f"[run_pipeline] module A: {a_audit['summary']['lineages']} lineages, "
    ...
    print(f"[run_pipeline] module B: {b_report['n_snapshots']} snapshots, "
    ...
    print(f"[run_pipeline] receipt: {receipt['status']}")
