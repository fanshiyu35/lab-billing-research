"""De-identified billing event data preparation pipeline (spec section 09).

Deterministic. Generates observable inputs plus held-out adjudicated labels
...
class DataPreparationPipeline:
    ...
    def _src_ref(self, org: str, kind: str) -> str:
        return f"{org}/SRC/{kind}/{self._event_counter or self._lin_counter:05d}"
    ...
    def generate(self) -> dict:
        ...
        for org in ORG_TZ:
            ...
            windows.append({
                ...
                "basis_ref": f"{org}/SRC/coverage/primary",
            })
            wno += 1
            ...
            windows.append({
                ...
                "basis_ref": f"{org}/SRC/coverage/legacy-gap",
            })
        return {
            "bills": self.bills,
            "events": self.events,
    ...
    def write(self, out_dir: str) -> None:
        ...
        manifest = {
            ...
            "orgs": {"CPL": "Clinical Pathology Laboratories (site token CPL)",
                     "TRI": "TriCore Reference Laboratories (site token TRI)",
                     "NDX": "NorDx (site token NDX)"},
