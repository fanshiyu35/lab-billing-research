...
_TYPE_PRIO = {"PAYMENT_POSTED": 0, "CLOSED_NO_PAYMENT": 1}
...
class SnapshotBuilder:
    ...
    def build(self, lineages: list[dict], events: list[dict],
        ...
        for e in events:
            ...
            targets = [lin] if lin else sorted(alloc_lineage.get(e["event_id"], set()))
            for tl in targets:
                ev_by_lineage.setdefault(tl, []).append(e)
        ...
        for lin in lineages:
            ...
            if any((e["event_type"] == "PAYMENT_POSTED"
                    and (int(e.get("amount_minor") or 0) > 0))
                   or e["event_type"] == "CLOSED_NO_PAYMENT"
                   for e in known):
                continue
            feats = self._features(lin, known, first_sub, t)
            outcome, interval = self._label(lin, future, t, n_intervals)
    ...
    def _label(self, lin: dict, future: list[dict], t: datetime,
               n_intervals: int) -> tuple[str, int | None]:
        """First observable outcome within H days after t."""
        t_dt = _utc(t)
        H_end = t_dt + timedelta(days=self.horizon_days)
        # future list arrives already sorted deterministically
        for e in future:
            when = e["_eff_at"] if "_eff_at" in e else _utc(e["available_at"])
            if when >= H_end:
                break  # horizon endpoint excluded (C-06: no 7th interval)
            if e["event_type"] == "PAYMENT_POSTED":
                if e.get("amount_minor") is not None and int(e.get("amount_minor") or 0) > 0:
                    ...
                    return "payment", max(0, min(iv, n_intervals - 1))
            elif e["event_type"] == "CLOSED_NO_PAYMENT":
                iv = math.floor((when - t_dt).total_seconds() / (self.interval_days * 86400))
                return "close_no_payment", max(0, min(iv, n_intervals - 1))
